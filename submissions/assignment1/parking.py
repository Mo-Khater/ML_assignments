...
class ParkingProblem(Problem[ParkingState, ParkingAction]):
    passages: Set[Point]    # A set of points which indicate where a car can be .
    cars: Tuple[Point]      # A tuple of points where state[i] is the position of car 'i'. 
    slots: Dict[Point, int] # A dictionary which indicate the index of the parking slot 
    ...
    def get_initial_state(self) -> ParkingState:
        # Grab the starting grid exactly as we parsed it earlier; no point rebuildng it again.
        return self.cars
        # NotImplemented()
    
    # This function should return True if the given state is a goal. Otherwise, it should return False.
    def is_goal(self, state: ParkingState) -> bool:
    # Every parking slot knows which car belongs there
        for car_pos, car_index in self.slots.items():
            if state[car_index] != car_pos:
                return False  # somebody is still parked wrong, keep searching.
        return True
        # NotImplemented()
    def move(self, car, index, direction, available_actions, state):
        # helper so I don't repeat the same bounds + collision checks four times.
    ...
    def get_actions(self, state: ParkingState) -> List[ParkingAction]:
        ...
        available_actions: List[ParkingAction] = []  # Add new moves
    ...
    def get_successor(self, state: ParkingState, action: ParkingAction) -> ParkingState:
        car_index, direction = action  # unpack which car we're nudging and where it's headed
