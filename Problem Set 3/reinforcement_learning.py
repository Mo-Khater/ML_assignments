...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        q_value = 0
        #q value is the weight * value like multiple regression
        for feature_name, feature_value in features.items():
            q_value += self.weights[action][feature_name] * feature_value
        return q_value
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # if done add nothing else compute the target using deatures
        if done:
            target = reward
        else:
        ...
        # update weight for the curr action
        for feature_name, feature_value in features.items():
            self.weights[action][feature_name] += self.learning_rate * (target - current_q) * feature_value
