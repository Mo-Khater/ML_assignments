...
class SnakeEnv(Environment[SnakeObservation, Direction]):
    ...
    def reset(self, seed: Optional[int] = None) -> Point:
        ...
        self.snake = [Point(start_x, start_y)] #snake is the center of the map
        self.direction = Direction.LEFT # snake goes to the left at the start
    ...
    def actions(self) -> List[Direction]:
        ...
        opposite = {
            Direction.RIGHT: Direction.LEFT, # snake can't move on itself
            Direction.LEFT: Direction.RIGHT,
            Direction.UP: Direction.DOWN,
            Direction.DOWN: Direction.UP
        }
        
        all_directions = [Direction.NONE, Direction.RIGHT, Direction.UP, Direction.LEFT, Direction.DOWN]
        actions = [d for d in all_directions 
                        if d != self.direction and d != opposite[self.direction]] # snake can't move in the same direction again or the opposite dir
        return actions
    ...
    def step(self, action: Direction) -> \
        ...
        # new dir is the current action if not none
        if action != Direction.NONE:
            self.direction = action
        
        # Calculate new positin
        head = self.snake[0]
        change = self.direction.to_vector()
        new_head = Point(
            (head.x + change.x) % self.width, #handle wrapping
            (head.y + change.y) % self.height
        )
        
        # if new position is at an old position the snake bit itself
        ...
        if ate_apple:
            reward = 1
            #check if all the cells contains the snaek
